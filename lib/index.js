...
var { parse } = require('@webpod/ingrid');
var fs = require('fs');

const isBin = f => fs.existsSync(f) && fs.lstatSync(f).isFile()
...
exports.lookup = function (query, callback) {
  ...
  return Exec(exeArgs, function (err, output) {
    if (err) {
      ...
      var processList = parseGrid(output.trim());
      var resultList = [];
...
function formatOutput(data) {
  ...
  data.forEach(function (d) {
    ...
    if (pid && cmd) {
      var c = cmd.findIndex((_v, i) => isBin(cmd.slice(0, i).join(''))) - 1
      var command = cmd.slice(0, c).join('');
      var args = '';
